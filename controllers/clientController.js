...
const getOverview = async (req, res) => {
  try {
    ...
    try {
      ...
      if (stats.length > 0) clientStats = stats;
    } catch (e) {
      console.error('Error fetching client stats:', e.message);
    }
    ...
    let contactStats = [{ total_contacts: 0, contacts_today: 0, contacts_last_7_days: 0 }];
    try {
      const [stats] = await pool.execute(
        `SELECT 
      ...
      if (stats.length > 0) contactStats = stats;
    } catch (e) {
      console.error('Error fetching contact stats:', e.message);
    }

    // --- 3. Invoice Stats (Clients having...) ---
    // Unpaid, Partially Paid, Overdue
    let invoiceStats = [{ clients_unpaid: 0, clients_partially_paid: 0, clients_overdue: 0 }];
    try {
      const [stats] = await pool.execute(
        `SELECT 
      ...
      if (stats.length > 0) invoiceStats = stats;
    } catch (e) {
      console.error('Error fetching invoice stats:', e.message);
    }

    // --- 4. Project Stats (Clients having...) ---
    let projectStats = [{ clients_open_projects: 0, clients_completed_projects: 0, clients_hold_projects: 0, clients_canceled_projects: 0 }];
    try {
      const [stats] = await pool.execute(
        `SELECT 
      ...
      if (stats.length > 0) projectStats = stats;
    } catch (e) {
      console.error('Error fetching project stats:', e.message);
    }

    // --- 5. Estimate Stats (Clients having...) ---
    let estimateStats = [{ clients_open_estimates: 0, clients_accepted_estimates: 0, clients_new_estimate_requests: 0, clients_estimates_in_progress: 0 }];
    try {
      const [stats] = await pool.execute(
        `SELECT 
      ...
      if (stats.length > 0) estimateStats = stats;
    } catch (e) {
      console.error('Error fetching estimate stats:', e.message);
    }
    ...
    try {
      ...
      console.error('Error fetching revenue stats:', e.message);
    }
